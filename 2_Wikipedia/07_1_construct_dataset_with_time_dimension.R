...
# bone_df <- data.frame(YEAR=character(),
#                          wikidataid=character(),
#                          wikiurl_basename_clean=character())
# for (k in seq_along(YEAR)){
#   IDs_df$YEAR <- as.character(YEAR[k])
#   bone_df <- rbind(bone_df, IDs_df)
# }
...
for (k in seq_along(YEAR)){
  ...
  # 
  # temp_df <- bone_df %>% merge(pageviews_df, by.x = c("wikiurl_basename_clean", "YEAR"), by.y = c("page_url_clean", "YEAR"), all.x = TRUE) %>%
...
nrow(revisions_editors_df)
names(revisions_editors_df)
nrow(nolangeds_df)
names(nolangeds_df)
