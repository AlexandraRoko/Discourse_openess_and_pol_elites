...
unique(NamesWPSpeeches$PARTY)


#Q27975838
#NamesAndSpeeches_full<- read.csv("./01_data/Plenarprotokolle/processed/NamesAndSpeeches_full.csv")

NamesWPSpeeches_groupedWP <- read.csv("./01_data/Plenarprotokolle/processed/NamesWPSpeeches_grouped_by_IDNameWP_with_AfD_later.csv")
NamesWPSpeeches_groupedYear <- read.csv("./01_data/Plenarprotokolle/processed/NamesWPSpeeches_grouped_by_IDNameYear_with_AfD_later.csv")
NamesWPSpeeches_grouped_month_year<- read.csv("./01_data/Plenarprotokolle/processed/NamesWPSpeeches_grouped_by_Month_Year_with_AfD_later.csv")

colnames(NamesWPSpeeches)

sum(is.na(NamesWPSpeeches_groupedYear$novelty) == TRUE) / length(NamesWPSpeeches_groupedYear$novelty)
...
merged_df[merged_df$name == "Alice Weidel"]

# merged_df <- merged_df[order(merged_df$politicianID),]
# length(unique(merged_df$wikidataid)) #2184 unique politicians
# merged_df <- merge(merged_df, PolID2Info_df[ , c("politicianID","lastName","firstName","positionShort")], by=c("politicianID"), all.x = TRUE)
# length(unique(merged_df$wikidataid)) #2184 unique politicians
...
#merged_df3 <- merged_df %>% left_join(MPs_and_parties[, c("wikidataid", "party", "session", "ROLE", "YEAR_OF_BIRTH")], by = c("wikidataid", "party", "session"))
...
nrow(merged_df[is.na(merged_df$ROLE),]) # 0 without position short
...
# number of persons where Party is missing and which are not guests
#length(merged_df$wikidataid[(is.na(merged_df$PARTY) == TRUE) & (merged_df$positionShort != "Guest")]) # 153 without name

# there are duplicates in the dataframe
invest <- merged_df[ , c("year", "wikidataid")]
invest <- invest%>% group_by_all %>% count
one_case <- subset(merged_df, merged_df$wikidataid=="Q2571121")
# remove duplicates 
merged_df <- merged_df %>% distinct(wikidataid, year, .keep_all = TRUE)
invest <- merged_df[ , c("year", "wikidataid")]
invest <- invest%>% group_by_all %>% count
one_case <- subset(merged_df, merged_df$wikidataid=="Q105676")


unique(merged_df$ethnicity) 
freq(merged_df$ethnicity) # 32% NANs
...
unique(merged_df$occupation)
freq(merged_df$occupation) # 0% NANs
...
#DATA07 <-subset(DATA06, is.na(DATA06$religion) != TRUE) # party
...
#### MY FA ANALYSIS
# for resonance
OLS0<-lm(resonance ~ fa_influence+sex+ETHN+religion+party+ROLE+year, data=DATA_FIN)
OLS1<-lm(resonance ~ fa_prominence+sex+ETHN+religion+party+ROLE+year, data=DATA_FIN)
stargazer(OLS0, OLS1, type="text")

FE1<-plm(resonance ~ fa_influence+year, data=DATA_FIN, index=c("wikidataid","year"), model = "within")

DATA_FIN_men <- subset(DATA_FIN, DATA_FIN$sex == "male") 
DATA_FIN_women <-  subset(DATA_FIN, DATA_FIN$sex == "female") 

FE1_M<-plm(resonance ~ fa_influence+year, data=DATA_FIN_men, index=c("wikidataid","year"), model = "within")
FE1_F<-plm(resonance ~ fa_influence+year, data=DATA_FIN_women, index=c("wikidataid","year"), model = "within")

stargazer(OLS0, OLS1, FE1, FE1_M, FE1_F, type="text")


# for novelty
OLS0<-lm(novelty ~ fa_influence+sex+ETHN+religion+party+ROLE+year, data=DATA_FIN)
OLS1<-lm(novelty ~ fa_prominence+sex+ETHN+religion+party+ROLE+year, data=DATA_FIN)
stargazer(OLS0, type="text")

FE1<-plm(novelty ~ fa_influence+year, data=DATA_FIN, index=c("wikidataid","year"), model = "within")

DATA_FIN_men <- subset(DATA_FIN, DATA_FIN$sex == "male") 
DATA_FIN_women <-  subset(DATA_FIN, DATA_FIN$sex == "female") 

FE1_M<-plm(novelty ~ fa_influence+year, data=DATA_FIN_men, index=c("wikidataid","year"), model = "within")
FE1_F<-plm(novelty ~ fa_influence+year, data=DATA_FIN_women, index=c("wikidataid","year"), model = "within")
...
# form2 <- resonance ~ fa_influence_fa+ROLE+year
# DATA_FIN.p <- pdata.frame(subset(DATA_FIN, select = c("wikidataid","year", "resonance", "fa_influence_fa", "ROLE", "year")), index = c("wikidataid","year"))
# mf2 <- model.frame(DATA_FIN.p, form2)
# 
# # no linear dependence in the pooling model's model matrix
# # (with intercept in the formula, there would be linear depedence)
# detect.lindep(model.matrix(mf2, model = "pooling"))
# modmat_FE2 <- model.matrix(mf2, model = "within")
# detect.lindep(modmat_FE2)
# mod_FE2 <- plm(form2, data = DATA_FIN.p, model = "within")
# detect.lindep(mod_FE2)
# alias(mod_FE2) 
# plm(form2, data = mf2, model = "within")$aliased 
# summary(mod_FE2)
# coeftest(mod_FE2, vcov = vcovHC, type = "HC1")
# aneweytest(form2, DATA_FIN.p)
# ??coeftest


stargazer(FE1, coeftest(FE1_rev),
          type="text")


stargazer(OLS0, OLS1, FE1, FE2, coeftest(FE1), coeftest(FE2),
          type="text")
...
stargazer(FE1, FE1_M, FE1_F, coeftest(FE1_M), coeftest(FE1_F), type="text")
...
stargazer(OLS0, OLS1,OLS0_withoutROLE, OLS1_withoutROLE,  type="latex")
...
stargazer(OLS0, OLS1, FE1, FE2, type="text")

stargazer( FE1, FE2,  coeftest(FE1), coeftest(FE2), type="text")
...
stargazer(FE1, FE1_M, FE1_F, type="text")

stargazer(FE1, FE1_M, FE1_F, type="latex")
